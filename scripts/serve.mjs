...
export function serve(options) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    if (pathname?.endsWith(".js")) {
      try {
        let fileToServe;
        if (pathname === "/discordplus.min.js") {
        ...
        } else if (pathname === "/discordplus.js") {
          const { config, context, timeTook } = await buildBundle();
  ...
  console.info(chalk.bold.yellowBright("Serving discord plus on:"));
  ...
  for (const netinterfaces of Object.values(netInterfaces)) {
    for (const details of netinterfaces || []) {
      ...
      console.info(`  http://${details.address}:${port}/discordplus.js`);
      console.info(`  http://${details.address}:${port}/discordplus.min.js`);
    }
  }
